...
void Akshay()
{
    ...
    // now only check if they are overlapping or not
    int a = mini[0],b = maxi[0];
    int cnt=0;

     rep(i,1,n){
        // Update a and b to the maximum valid range seen so far
        a = max(a, mini[i]);
        b = min(b, maxi[i]);

        // If a and b don't overlap for the current pile, update and count
        if (a > b) {
            cnt++;
            // Update a and b to the current pile's range
            a = mini[i];
            b = maxi[i];
        }
    }
