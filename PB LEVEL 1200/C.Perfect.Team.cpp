...
void Akshay() {
    ...
    int low = 0;
    int high = min((c + m + x) / 3, min(c, m));
    int ans = 0;

    // Lambda function to check if we can form 'mid' number of teams
    auto solve = [&](int c, int m, int x, int mid) {
    ...
    // Binary search for the maximum number of teams
    while (low <= high) {
        int mid = low + (high - low) / 2;
        if (solve(c, m, x, mid)) {
            ans = mid;  // we can form 'mid' teams, so try for more
            low = mid + 1;
        } else {
            high = mid - 1;
        }
